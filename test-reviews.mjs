...
async function main() {
  ...
  // 1. Fetch a few reviews from Outscraper (reviewsLimit = reviews per place)
  console.log(`📥 Fetching ${TEST_LIMIT} reviews from Outscraper for place ${GOOGLE_PLACE_ID}...`);
  const outscraperRes = await fetch(
    `https://api.app.outscraper.com/maps/reviews-v3?query=${GOOGLE_PLACE_ID}&reviewsLimit=${TEST_LIMIT}&skip=0&async=false`,
  ...
  if (!reviews.length) {
    console.log('\n⚠️  No reviews returned. Double-check GOOGLE_PLACE_ID and OUTSCRAPER_API_KEY.');
    return;
  }
